...
func SetDefaultRuleset(uuid string) (err error) {
  ...
  for x := range rsets {
    ...
    if rsets[x]["default"] == "" && rsets[x]["type"] == "local" {
      ...
      if err != nil {
        logs.Error("ruleset/SetDefaultRuleset: %s", err.Error())
        return err
      }
    }
...
func AddNewRuleset(data map[string]map[string]string) (duplicated []byte, err error) {
  ...
  for x := range data {
    for rsetID := range rsets {
      if rsetID == data[x]["uuid"] && !rulesetCreated {
        rulesetCreated = true
      }
    }
  ...
  for x := range data {
    ...
    for id := range rsets {
      if rsets[id]["sourceUUID"] == data[x]["uuid"] {
        ...
        if err != nil {
          logs.Error("ruleset/ModifyRuleset -- delete files locally for update error: %s", err.Error())
          return nil, err
        }
        //delete rule_file by uuid
        err = ndb.DeleteRuleFilesByUuid(id)
        if err != nil {
          logs.Error("ruleset/ModifyRuleset -- delete files at database for update error: %s", err.Error())
          return nil, err
        }
      }
    }
