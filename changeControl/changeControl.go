...
func InsertChangeControl(values map[string]string) (err error) {
  ...
  if err != nil {
    logs.Error("changeChangeControl/InsertChangeControl error readding data from main.conf: " + err.Error())
    return
  }
  ...
  err = ndb.InsertChangeControl(uuid, "deviceName", deviceName)
  if err != nil {
    logs.Error("Error inserting ChangeControl database value: " + err.Error())
    return err
  }
  err = ndb.InsertChangeControl(uuid, "time", timeFormated)
  if err != nil {
    logs.Error("Error inserting ChangeControl database value: " + err.Error())
    return err
  }
  ...
  for x := range values {
    err = ndb.InsertChangeControl(uuid, x, values[x])
    if err != nil {
      logs.Error("Error inserting ChangeControl database value: " + err.Error())
      return err
    }
...
func ChangeControlInsertData(err error, desc string, username string) {
  ...
  if controlError != nil {
    logs.Error(desc + " controlError: " + controlError.Error())
  }
}
