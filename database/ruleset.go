...
func InsertRulesetSchedule(uuid string, key string, value string) (err error) {
    ...
    stmt, err := Rdb.Prepare("insert into ruleset_schedule (schedule_uniqueid, schedule_param, schedule_value) values (?,?,?);")
    if err != nil {
...
func UpdateSchedule(uuid string, param string, value string)(err error){
	updateRulesetNode, err := Rdb.Prepare("update ruleset_schedule set schedule_value = ? where schedule_uniqueid = ? and schedule_param = ?;")
        if (err != nil){
