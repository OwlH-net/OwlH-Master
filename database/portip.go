...
func ObtainPortIp(uuid string)(ip string, port string, err error)  {
    ...
    if err != nil {logs.Error("DB NODE obtainPortIp ipNode -> ndb.Db.QueryRow error: %s", err.Error()); return "","",err}
...
func GetTokenByUuid(uuid string)(err error)  {
    if Db == nil {logs.Error("GetTokenByUuid obtainPortIp -> Error conexión DB"); return err}
