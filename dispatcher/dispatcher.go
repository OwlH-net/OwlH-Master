...
func Init() {
  ...
  for {
    ...
    if err != nil {
      logs.Error("Error PingPlugins for check dispatcher status: %s", err.Error())
    }
    if pluginData["dispatcher"]["status"] == "disabled" {
      t, err := utils.GetKeyValueString("loop", "dispatcher")
      if err != nil {
        logs.Error("Search Error: Cannot load dispatcher information.")
      }
...
func GetKeepPcap() (status bool, err error) {
  ...
  if err != nil {
    logs.Error("Error GetKeepPcap importing from main.conf: " + err.Error())
    return true, err
  }

  keepBool, err := strconv.ParseBool(keepPcacpStatus)
  if err != nil {
    logs.Error("Error GetKeepPcap Parsing to bool: " + err.Error())
    return true, err
  }
  return keepBool, nil
}
...
func GetOutputQueue() (path string, err error) {
  ...
  if err != nil {
    logs.Error("Error GetOutputQueue importing from main.conf: " + err.Error())
    return "", err
  }
...
func dispatch(theList listOfNodesAndFolders) {
  ...
  for i := 0; i < len(theList.Folders); i++ {
    ...
    if err != nil {
      logs.Error("Error GetKeepPcap at dispatch: " + err.Error())
    }
    ...
    if pool != nil {
      for _, file := range files {
        ...
        if !KeepPcapStatus {
          ...
          if err != nil {
            logs.Error("CopyFileToNode cleaning pcap: " + err.Error())
          }

          err = os.Remove(theList.Folders[i].Fpath + file.Name())
          if err != nil {
            logs.Error("Error Removing =-> " + err.Error())
          }
        }
        latest += 1
      }
